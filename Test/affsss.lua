...
local function createFluentUI()
    ...
    -- Tabs
    local Tabs = {
        Farming = Window:AddTab({ Title = "Farming", Icon = "carrot" }),
    ...
    do
        ...
        local ArtiDropdownOne = Tabs.Arte:AddDropdown("ArtiDropdownOne", {
        ...
        local ArtiDropdown = Tabs.Arte:AddDropdown("ArtiDropdownTwo", {
        ...
        Tabs.Arte:AddButton({
            Title = "Equip Slot 1",
            Description = "Equip selected slot 1 artifact",
            Callback = function()
                if fuckingdata.artiTwo then
                    equipArtifactOne(fuckingdata.artiTwo)
                    Fluent:Notify({
                        Title = "Success",
                        Content = "Equipped " .. fuckingdata.artiTwo,
                        Duration = 3
                    })
                else
                    Fluent:Notify({
                        Title = "Error",
                        Content = "No artifact selected for slot 1",
                        Duration = 3
                    })
                end
            end
        ...
        Tabs.Arte:AddButton({
            ...
            Callback = function()
                if fuckingdata.artiSelected then
                    ...
                    Fluent:Notify({
                        Title = "Error",
                        Content = "No artifact selected for slot 2",
                        Duration = 3
                    })
                end
            end
...
local teamsTable = {}
local StoreModule, GameData, allData, petTeams


pcall(function()
    StoreModule = require(game:GetService("ReplicatedStorage").ModuleScripts.LocalDairebStore)
    GameData = StoreModule.GetStoreProxy("GameData")
    allData = GameData:GetData() or {}
    petTeams = allData["PetTeams"] or {}
...
getgenv().TeamSelected = nil

function equipTeamByName(teamName)
    if not petTeams then return end
    for _, team in ipairs(petTeams) do
        if team.Name == teamName then
            local uids = team.UIDs or {}
    ...
    warn("No found team:", teamName)
end


local function addTeamsUI(Tabs)
    if #teamsTable > 0 then
        ...
        Tabs.Team:AddButton({
            ...
            Callback = function()
                if getgenv().TeamSelected then
                    equipTeamByName(getgenv().TeamSelected)
                    Fluent:Notify({
                        Title = "Team Equipped",
                        Content = "Changed to" .. getgenv().TeamSelected,
                        Duration = 3
                    })
                else
                    Fluent:Notify({
                        Title = "No Team Selected",
                        Content = "Go!! Select it",
                        Duration = 3
                    })
                end
            end
...
local originalCreateFluentUI = createFluentUI
createFluentUI = function()
    local Window = originalCreateFluentUI()
    local Tabs = {
        Team = Window:AddTab({ Title = "Teams | Test", Icon = "gift" })
    }
    addTeamsUI(Tabs)
    return Window
end
